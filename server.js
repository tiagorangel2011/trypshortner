...
app.post("/api/create", body, (req, res) => {
  ...
  // process.env.KEY
    ...
    return res
      .status(400)
      .json({ success: false, error: "Invalid or inexistent access key" });
  }
  ...
  if (!url)
    return res
      .status(400)
      .json({ success: false, error: "Please enter a URL" });
  ...
  if (url.includes(req.get("host")))
    return res.status(400).json({
      success: false,
      error: "Cannot shorten the shortner domain"
    });
...
app.post("/api/delete", body, (req, res) => {
  ...
  if (!slug || !token)
    return res
      .status(400)
      .json({ success: false, error: "There are missing fields" });
  ...
  if (!result)
    return res
      .status(400)
      .json({ success: false, error: "Invalid slug or delete token" });
